...
if __name__ == "__main__":
    ...
    for epoch in range(opt.num_epochs):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            netD.zero_grad()
            # Format batch
            # real_cpu : (128,3,64,64)
            real_cpu = data[0].to(device)
            # Add noise
            eps = torch.randn_like(real_cpu)
            noise_real = eps * opt.noise_scale
            real_cpu_noise = real_cpu + noise_real
            b_size = real_cpu.size(0)
            label = torch.full(
                (b_size,), real_label, dtype=torch.float, device=device
            )
            # Forward pass real batch through D
            output = netD(real_cpu_noise).view(-1)
            ...
            noise_fake = eps * opt.noise_scale
            fake_noise = fake + noise_fake
    ...
    torch.save(netD.state_dict(), f'results/{opt.cat_id}/model/discriminator')
    torch.save(netG.state_dict(), f'results/{opt.cat_id}/model/generator')
